...
namespace ieee802_11 {
	...
	ofdm_allocator_impl::ofdm_allocator_impl(
		int fft_len,
		const std::vector<std::vector<int> > &occupied_carriers,
		const std::vector<std::vector<int> > &pilot_carriers,
		const std::vector<std::vector<gr_complex> > &pilot_symbols,
	...
	int
	ofdm_allocator_impl::calculate_output_stream_length(const gr_vector_int &ninput_items)
	{
	...
	int
	ofdm_allocator_impl::work (int noutput_items,
